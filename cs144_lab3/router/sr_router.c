...
void sr_handle_host_unreachable_ip_packet(struct sr_instance *sr, uint8_t *packet, unsigned int len, char *interface)
{
 ...
 /* Get the ethernet header */
  ...
  struct sr_if* client_interface = sr_get_interface(sr, client_routing_entry->interface);
  struct sr_if* server_interface = sr_get_interface(sr, interface);
  ...
  if (sr_send_packet(sr, new_packet, new_packet_len, client_interface->name) != 0) {
    fprintf(stderr, "ERROR: Packet sent unsuccessfully\n");
  }
  free(new_packet);
}
